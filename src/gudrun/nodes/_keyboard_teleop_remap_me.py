...
class Teleop(object):
    ...
    @property
    def steering(self):
        return self.message.angular.z

    @steering.setter
    def steering(self, newvalue):
        self.message.angular.z = newvalue
        self.publish()

    @property
    def throttle(self):
        return self.message.linear.x

    @throttle.setter
    def throttle(self, newvalue):
        self.message.linear.x = newvalue
        self.publish()
...
class KeyboardTeleop(Teleop):
    ...
    def spin(self):
        ...
        def inc(steering=True, direction=+1):
            if steering:
                self.steering += STEERING_INCREMENT * direction
                print('to', self.steering)
            else:
                self.throttle += THROTTLE_INCREMENT * direction
                print('to', self.throttle)
        ...
        actions = dict(
            ...
            c=('Center steering.', self.center),
            h=('Display key listing.', display_help),
            )    
        actions[' '] = ('Cut throttle.', self.stop)
        actions['\t'] = ('Cut throttle.', self.stop)
        ...
        while True:
            ...
            if char in actions:
                ...
                print(label, end='\n' if action in [None, display_help, self.center, self.stop] else ' ')
                if action is None:
                    break
                action()
            # else:
